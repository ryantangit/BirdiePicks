...
export default async function SummonerPage(props: SummonerPageProps) {
  ...
  return (
    <>
      ...
      />
      <MatchHistory
        puuid={result.puuidResult.puuid}
        routeRegion={props.regionID[0]}
      />
    </>
  );
...
async function fetchSummonerProfile(regionID: string[]) {
  ...
  const puuidResult = await riotQuery.accountQuery.getPuuid(
    gameIdGameTag[0],
    gameIdGameTag[1],
    regionRoute,
  );
  ...
  const summonerResult = await riotQuery.summonerQuery.getSummonerInfo(
    puuidResult.puuid,
    regionRoute,
  );
  if (!summonerResult) {
